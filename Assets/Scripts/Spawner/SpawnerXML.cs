...
public class SpawnerXML : MonoBehaviour
{
    ...
    // Loads XMLContainer from path (NOTE: only run if the file definitely exists)
    XMLContainer Load(string path)
    {
        // Create a serialzer of type XMLContainer
        XmlSerializer serializer = new XmlSerializer(typeof(XMLContainer));
    ...
    public void Save()
    {
        ...
        for (int i = 0; i < objects.Count; i++)
        {
            ...
            //SET data's rotatin to item's rotation
